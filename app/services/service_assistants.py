...
async def create_assistant_service(assistant: CreateAssistantRequest):
...
def list_openai_assistants(order="desc", limit=20):
    url = "https://api.openai.com/v1/assistants"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {OPENAI_API_KEY}",
...
def modify_openai_assistant(assistant_id: str, data: Dict[str, Any]):
...
def delete_openai_assistant(assistant_id: str) -> dict:
    url = f"https://api.openai.com/v1/assistants/{assistant_id}"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {OPENAI_API_KEY}",
